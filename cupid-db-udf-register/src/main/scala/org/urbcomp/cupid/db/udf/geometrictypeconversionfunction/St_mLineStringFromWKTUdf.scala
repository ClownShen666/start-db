...
class St_mLineStringFromWKTUdf extends AbstractUdf {

  override def name(): String = "st_mLineStringFromWKT"

  override def registerEngines(): List[DataEngine.Value] = List(Calcite, Spark)
  @throws[ParseException]
  def evaluate(wktString: String): MultiLineString = {
    if (wktString == null) null
    else {
      st_castToMLineString(st_geomFromWKT(wktString))
    }
  }

  def udfSparkEntries: List[String] = List("udfWrapper")

  def udfWrapper: (String) => MultiLineString = evaluate
}
