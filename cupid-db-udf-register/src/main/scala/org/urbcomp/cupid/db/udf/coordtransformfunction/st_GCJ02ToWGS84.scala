...
class st_GCJ02ToWGS84 extends AbstractUdf {
  ...
  def evaluate(trajectory: Trajectory): Trajectory = {
    MatchUtil.MathTRR(new GCJ02ToWGS84Transformer).trajectoryTransform(trajectory)
  }

  def evaluate(roadNetwork: RoadNetwork): RoadNetwork = {
    MatchUtil.MathTRR(new GCJ02ToWGS84Transformer).roadNetworkTransform(roadNetwork)
  }

  def evaluate(roadSegment: RoadSegment): RoadSegment = {
    MatchUtil.MathTRR(new GCJ02ToWGS84Transformer).roadSegmentTransform(roadSegment)
  }
