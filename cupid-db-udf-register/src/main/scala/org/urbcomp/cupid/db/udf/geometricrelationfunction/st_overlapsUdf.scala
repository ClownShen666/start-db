...
class st_overlapsUdf extends ScalarFunction with AbstractUdf {
  override def name(): String = "st_overlaps"
  ...
  def eval(geom1: Geometry, geom2: Geometry): java.lang.Boolean = {
    if (geom1 == null || geom2 == null) null
      ...
      preparedGeom1 match {
        case Some(s) => s.overlaps(geom2)
        case _       => geom1.overlaps(geom2)
      }
    }
  }
