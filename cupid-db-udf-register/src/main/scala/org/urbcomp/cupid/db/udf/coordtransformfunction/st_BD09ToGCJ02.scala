...
class st_BD09ToGCJ02 extends AbstractUdf {
  ...
  def evaluate(trajectory: Trajectory): Trajectory = {
    MatchUtil.MathTRR(new BD09ToGCJ02Transformer).trajectoryTransform(trajectory)
  }

  def evaluate(roadNetwork: RoadNetwork): RoadNetwork = {
    MatchUtil.MathTRR(new BD09ToGCJ02Transformer).roadNetworkTransform(roadNetwork)
  }

  def evaluate(roadSegment: RoadSegment): RoadSegment = {
    MatchUtil.MathTRR(new BD09ToGCJ02Transformer).roadSegmentTransform(roadSegment)
  }
