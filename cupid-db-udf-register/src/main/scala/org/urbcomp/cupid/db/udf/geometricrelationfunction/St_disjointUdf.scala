...
class St_disjointUdf extends AbstractUdf {
  override def name(): String = "st_disjoint"
  ...
  def evaluate(geom1: Geometry, geom2: Geometry): java.lang.Boolean = {
    if (geom1 == null || geom2 == null) null
      ...
      preparedGeom1
        .map(preparedGeometry => preparedGeometry.disjoint(geom2))
        .getOrElse(geom1.disjoint(geom2))
    }
  }
