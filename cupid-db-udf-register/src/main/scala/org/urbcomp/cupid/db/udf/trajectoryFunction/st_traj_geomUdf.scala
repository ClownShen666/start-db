...
class st_traj_geomUdf extends ScalarFunction with AbstractUdf {
  ...
  def eval(@DataTypeHint(value = "RAW", bridgedTo = classOf[Trajectory]) trajectory: Trajectory): LineString = {
  ...
  def udfWrapper1: Trajectory => LineString = eval
}
