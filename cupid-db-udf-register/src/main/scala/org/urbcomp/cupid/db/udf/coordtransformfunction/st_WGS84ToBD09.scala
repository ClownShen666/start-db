...
class st_WGS84ToBD09 extends AbstractUdf {
  ...
  def evaluate(trajectory: Trajectory): Trajectory = {
    MatchUtil.MathTRR(new WGS84ToBD09Transformer).trajectoryTransform(trajectory)
  }

  def evaluate(roadNetwork: RoadNetwork): RoadNetwork = {
    MatchUtil.MathTRR(new WGS84ToBD09Transformer).roadNetworkTransform(roadNetwork)
  }

  def evaluate(roadSegment: RoadSegment): RoadSegment = {
    MatchUtil.MathTRR(new WGS84ToBD09Transformer).roadSegmentTransform(roadSegment)
  }
}
