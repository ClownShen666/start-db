...
class st_WGS84ToBD09 extends ScalarFunction with AbstractUdf {
  ...
  def eval(@DataTypeHint(value = "RAW", bridgedTo = classOf[Point]) st: Point): Point = {
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[Point]
  }
  ...
  def eval(
    ...
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[LineString]
  }
  ...
  def eval(@DataTypeHint(value = "RAW", bridgedTo = classOf[Polygon]) st: Polygon): Polygon = {
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[Polygon]
  }
  ...
  def eval(
    ...
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[MultiPoint]
  }
  ...
  def eval(
    ...
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[MultiLineString]
  }
  ...
  def eval(
    ...
    if (st == null) return null
    MatchUtil.MatchCoordinate(new WGS84ToBD09Transformer, st).asInstanceOf[MultiPolygon]
  }
