...
class StayPointSegmentUdtf extends AbstractUdtf with Serializable {
  override def name(): String = "st_traj_stayPointSegment"

  override def inputColumnsCount: Int = 3

  override def outputColumns(): List[(String, SqlTypeName)] = List(
    ("subTrajectory", SqlTypeName.TRAJECTORY)
  )
  ...
  override def udtfImpl(objects: Seq[AnyRef]): Array[Array[AnyRef]] = {
    ...
    val maxDistInMeter = objects(2).toString.toDouble

    val segment = new org.urbcomp.start.db.algorithm.trajectorysegment.StayPointSegment(
      maxStayTimeInSecond,
      maxDistInMeter
    )
    val result = segment.segment(trajectory)
    result
      .map(subTraj => {
