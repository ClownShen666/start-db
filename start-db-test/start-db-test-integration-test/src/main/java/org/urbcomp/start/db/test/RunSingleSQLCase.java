...
import static org.urbcomp.start.db.test.CompareResult.*;
import static org.urbcomp.start.db.test.GetData.*;
import static org.urbcomp.start.db.test.AutoWriteActual.*;
    ...
    public static Connection getConnect() throws Exception {
        ...
        try {
            conn = DriverManager.getConnection(
                "jdbc:start-db:url=http://localhost:8000",
                "root",
                "start-db"
            );
        } catch (Exception e) {
            throw new Exception("start-db 创建connection失败");
        }
        return conn;
    }
