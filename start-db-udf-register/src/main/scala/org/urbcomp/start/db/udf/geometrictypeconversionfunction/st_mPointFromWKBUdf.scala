...
class st_mPointFromWKBUdf extends ScalarFunction with AbstractUdf {

  override def name(): String = "st_mPointFromWKB"

  override def registerEngines(): List[DataEngine.Value] = List(Calcite, Spark, Flink)
  @throws[ParseException]
  @DataTypeHint(value = "RAW", bridgedTo = classOf[MultiPoint])
  def eval(wkb: Array[Byte]): MultiPoint = {
    if (wkb == null) null
    else {
  ...
  def udfWrapper: Array[Byte] => MultiPoint = eval
