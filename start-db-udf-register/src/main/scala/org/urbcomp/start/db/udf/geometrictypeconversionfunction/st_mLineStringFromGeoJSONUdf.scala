...
class st_mLineStringFromGeoJSONUdf extends ScalarFunction with AbstractUdf {

  override def name(): String = "st_mLineStringFromGeoJSON"

  override def registerEngines(): List[DataEngine.Value] = List(Calcite, Spark, Flink)
  @throws[ParseException]
  @DataTypeHint(value = "RAW", bridgedTo = classOf[MultiLineString])
  def eval(geoJson: String): MultiLineString = {
