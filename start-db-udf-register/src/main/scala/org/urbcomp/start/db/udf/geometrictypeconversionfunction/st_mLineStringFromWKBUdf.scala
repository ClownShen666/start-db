...
class st_mLineStringFromWKBUdf extends ScalarFunction with AbstractUdf {

  override def name(): String = "st_mLineStringFromWKB"

  override def registerEngines(): List[DataEngine.Value] = List(Calcite, Spark, Flink)
  @throws[ParseException]
  @DataTypeHint(value = "RAW", bridgedTo = classOf[MultiLineString])
  def eval(wkb: Array[Byte]): MultiLineString = {
    if (wkb == null) null
    else {
  ...
  def udfWrapper: Array[Byte] => MultiLineString = eval
