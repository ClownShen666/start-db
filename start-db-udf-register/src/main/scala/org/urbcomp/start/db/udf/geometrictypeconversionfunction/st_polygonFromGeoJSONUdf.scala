...
class st_polygonFromGeoJSONUdf extends ScalarFunction with AbstractUdf {

  override def name(): String = "st_polygonFromGeoJSON"

  override def registerEngines(): List[DataEngine.Value] = List(Calcite, Spark, Flink)
  @throws[ParseException]
  @DataTypeHint(value = "RAW", bridgedTo = classOf[Polygon])
  def eval(geoJson: String): Polygon = {
    if (geoJson == null) null
    else {
      castToPolygon(geomFromGeoJSON(geoJson))
    }
  }
