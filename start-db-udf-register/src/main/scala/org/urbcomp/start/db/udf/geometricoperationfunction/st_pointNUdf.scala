...
class st_pointNUdf extends ScalarFunction with AbstractUdf {

  override def name(): String = "st_pointN"
  ...
  @DataTypeHint(value = "RAW", bridgedTo = classOf[Point])
  def eval(@DataTypeHint(value = "RAW", bridgedTo = classOf[Point]) geom: Point, n: Int): Point =
    process(geom, n)
  ...
  private def process(geom: Geometry, n: Int): Point = {
    if (geom == null) null
    else
      geom match {
        case string: LineString =>
          if (n > 0 && n <= geom.getNumPoints)
            string.getPointN(n - 1)
          else if (n < 0 && n + geom.getNumPoints >= 0)
            string.getPointN(n + geom.getNumPoints)
          else null
        case _ => null
      }
  }
  ...
  def udfWrapper: (Geometry, Int) => Point = eval
}
