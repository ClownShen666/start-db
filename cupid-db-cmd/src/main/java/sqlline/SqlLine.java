...
public class SqlLine {
     ...
     * @throws IOException on error
     */
    public static Status mainWithInputRedirection(String[] args, InputStream inputStream)
        throws IOException {
    ...
    Status initArgs(String[] args, DispatchCallback callback) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (args[i].startsWith("--")) {
                ...
                if (parts.length > 0) {
                    ...
                    if (BuiltInProperty.valueOf(parts[0], true) == BuiltInProperty.SILENT) {
                        shouldBeSilent = true;
                        continue;
        ...
        if (appConfig != null) {
            dispatch(COMMAND_PREFIX + "appconfig " + appConfig, new DispatchCallback());
        }
        ...
        // cupid modify start
            ...
            String com = COMMAND_PREFIX
                + "connect "
                + (driver == null || driver.trim().isEmpty() ? "" : "-p driver " + driver + " ")
        ...
        if (nickname != null) {
            dispatch(
                COMMAND_PREFIX + "nickname " + escapeAndQuote(nickname),
                new DispatchCallback()
            );
        }

        if (logFile != null) {
            dispatch(COMMAND_PREFIX + "record " + escapeAndQuote(logFile), new DispatchCallback());
        }
        ...
        if (commandHandler != null) {
            ...
            dispatch(COMMAND_PREFIX + "commandhandler " + sb.toString(), new DispatchCallback());
        }

        if (promptHandler != null) {
            dispatch(COMMAND_PREFIX + "prompthandler " + promptHandler, new DispatchCallback());
        }
    ...
    private boolean makeSilent(boolean shouldBeSilent) {
        ...
        for (Map.Entry<BuiltInProperty, String> entry : map.entrySet()) {
            if (!getOpts().set(entry.getKey().propertyName(), entry.getValue(), true)) {
                return false;
            }
        }
        return true;
     ...
     *         history file or start console reader
     */
    public Status begin(String[] args, InputStream inputStream, boolean saveHistory)
        throws IOException {
        try {
            getOpts().load();
        } catch (Exception e) {
        ...
        if (runningScript) {
            try {
                FileInputStream scriptStream = new FileInputStream(getOpts().getRun());
    ...
    public LineReader getConsoleReader(InputStream inputStream, History fileHistory)
        throws IOException {
        ...
        if (inputStream != null) {
            terminal = terminalBuilder.streams(inputStream, getOutputStream()).build();
        } else {
        ...
        if (inputStream == null) {
            lineReader = lineReaderBuilder.appName("sqlline")
    ...
    private void addSqlLineWidgets(LineReader lineReader) {
        ...
        addWidget(lineReader, this::toggleAutopairWidget, "AUTOPAIRING", alt(ctrl('p')));
        addWidget(lineReader, this::nextColorSchemeWidget, "CHANGE_COLOR_SCHEME", alt('h'));
        addWidget(lineReader, this::toggleLineNumbersWidget, "TOGGLE_LINE_NUMBERS", alt(ctrl('n')));
    }
    ...
    boolean toggleLineNumbersWidget() {
        getOpts().set(BuiltInProperty.SHOW_LINE_NUMBERS, !getOpts().getShowLineNumbers());
    ...
    public void debug(String msg) {
        if (getOpts().getVerbose()) {
            output(new AttributedString(msg, AttributedStyles.BLUE), true, errorStream);
        }
    }
    ...
    public void output(AttributedString msg, boolean newline, PrintStream out) {
        if (getOpts().getColor()) {
            final String ansiMsg = getTerminal() == null ? msg.toAnsi() : msg.toAnsi(getTerminal());
    ...
    public String[][] splitCompound(
        ...
        while (n > 0 && (Character.isWhitespace(chars[n - 1]) || chars[n - 1] == ';')) {
            --n;
        }
                ...
                case QUOTED:
                    ...
                    if (c == dialect.getCloseQuote()) {
                        if (i < n && chars[i] == dialect.getCloseQuote()) {
                            ...
                            state = SPACE;
                            final String word = String.copyValueOf(chars, idStart, i - idStart - 1);
                            current.add(
                                keepSqlIdentifierQuotes
    ...
    public String[] split(String line, String delim, int limit) {
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            } else if (i > 0 && line.regionMatches(i - delim.length(), delim, 0, delim.length())) {
                if (inQuotes) {
                    continue;
                }
                tokenStart = i;
        ...
        if (lastProcessedIndex != line.length() - 1 && (limit == 0 || limit > tokens.size())
    ...
    String wrap(String toWrap, int len, int start) {
        ...
        for (StringTokenizer tok = new StringTokenizer(toWrap, " "); tok.hasMoreTokens();) {
            ...
            if (index >= 0) {
                ...
                buff.append(line).append(' ').append(next, 0, index).append(SEPARATOR).append(head);
                line.setLength(0);
    ...
    void handleSQLException(SQLException e) {
        ...
        for (SQLException nested = e.getNextException(); nested != null && nested != e; nested =
            nested.getNextException()) {
    ...
    Set<Driver> scanDrivers() {
        ...
        if (appConfig.allowedDrivers == null || !appConfig.allowedDrivers.isEmpty()) {
            Set<String> driverClasses = appConfig.allowedDrivers == null
                ? Collections.emptySet()
                : new HashSet<>(appConfig.allowedDrivers);
    ...
    int print(ResultSet rs, DispatchCallback callback) throws SQLException {
        ...
        if ("csv".equals(format)) {
            final SeparatedValuesOutputFormat csvOutput = (SeparatedValuesOutputFormat) f;
            if (csvOutput.separator == null && getOpts().getCsvDelimiter() != null
                || (csvOutput.separator != null
                    ...
                    || csvOutput.quoteCharacter != getOpts().getCsvQuoteCharacter())) {
                f = new SeparatedValuesOutputFormat(
                    this,
                    getOpts().getCsvDelimiter(),
                    getOpts().getCsvQuoteCharacter()
                );
                Map<String, OutputFormat> updFormats = new HashMap<>(getOutputFormats());
    ...
    void runBatch(List<String> statements) {
        try (Statement stmnt = createStatement()) {
            ...
            output(
                new AttributedStringBuilder().append(rpad("COUNT", 8), AttributedStyle.BOLD)
            ...
            for (int i = 0; i < counts.length; i++) {
                output(
                    new AttributedStringBuilder().append(rpad(counts[i] + "", 8))
    ...
    public int runCommands(List<String> cmds, DispatchCallback callback) {
        ...
        try {
            ...
            for (String cmd : cmds) {
                info(
                    new AttributedStringBuilder().append(rpad(index++ + "/" + size, 13))
                        .append(cmd)
                        .toAttributedString()
                );
    ...
    void outputProperty(String key, String value) {
        output(
            new AttributedStringBuilder().append(rpad(key, 20), AttributedStyles.GREEN)
                .append(value)
                .toAttributedString()
        );
    }
    ...
    public void setErrorStream(OutputStream errorStream) {
        try {
            this.errorStream = new PrintStream(errorStream, true, StandardCharsets.UTF_8.name());
        } catch (UnsupportedEncodingException e) {
            handleException(e);
        }
    }
    ...
    private class Config {
        ...
        Config(
            Collection<String> knownDrivers,
            SqlLineOpts opts,
            ...
            this.allowedDrivers = knownDrivers == null
                ? null
                : Collections.unmodifiableSet(new HashSet<>(knownDrivers));
            this.opts = opts;
            this.commandHandlers = Collections.unmodifiableList(new ArrayList<>(commandHandlers));
