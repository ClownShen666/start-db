  ...
  def join(other: GeoMesaRelation, condition: Expression): Option[GeoMesaJoinRelation] = {
    val opt = for { p <- partitioned; o <- other.partitioned } yield {
      val toJoin = if (p.envelopes == o.envelopes) {
        ...
        logger.warn(
          "Joining across two relations that are not partitioned by the same scheme - unable to optimize"
        )
        None
      }
      toJoin.map { rel =>
        GeoMesaJoinRelation(
          ...
          this,
          rel,
          StructType(schema.fields ++ rel.schema.fields),
          condition
        )
      }
    }
...
object GeoMesaRelation extends LazyLogging {
  ...
  def apply(sqlContext: SQLContext, params: Map[String, String]): GeoMesaRelation = {
    val name = params.getOrElse(
      GEOMESA_SQL_FEATURE,
      throw new IllegalArgumentException(
        s"Feature type must be specified with '$GEOMESA_SQL_FEATURE'"
      )
    )
  ...
  def apply(
      ...
      params.get(key) match {
        ...
        case Some(v) =>
          try {
            ...
            case NonFatal(e) =>
              logger.error(s"Error evaluating param '$key' with value '$v':", e); default
          }
      }
    }
    ...
    val partitioned = if (!get[Boolean]("spatial", _.toBoolean, false)) {
      None
    } else {
      val raw = rawRDD
      val bounds: Envelope = params.get("bounds") match {
        ...
        case Some(b) =>
          try {
            WKTUtils.read(b).getEnvelopeInternal
          } catch {
            case NonFatal(e) =>
              throw new IllegalArgumentException(s"Error reading provided bounds '$b':", e)
          }
      }
      ...
      Some(
        PartitionedRDD(rdd, raw, envelopes, partitions, get[Boolean]("cover", _.toBoolean, false))
      )
    }

    val cached = if (!get[Boolean]("cache", _.toBoolean, false)) {
      None
    } else {
      ...
      if (!DataStoreFinder.getAvailableDataStores.asScala.exists(_.canProcess(check))) {
        throw new IllegalArgumentException(
          "Caching requires the GeoCQEngineDataStore to be available on the classpath"
        )
      }
