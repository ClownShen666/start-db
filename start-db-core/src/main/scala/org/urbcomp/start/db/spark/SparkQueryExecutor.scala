...
import org.urbcomp.start.spark.jts._
import org.urbcomp.start.db.spark.res.SparkResultExporterFactory
import org.urbcomp.start.db.udf.DataEngine.Spark
import org.urbcomp.start.db.udf.{DataEngine, UdfFactory}
import org.urbcomp.start.db.udtf.AbstractUdtf
import org.urbcomp.start.db.util.{LogUtil, MetadataUtil, SparkSqlParam, SqlParam}
...
object SparkQueryExecutor {
  ...
  def execute(param: SparkSqlParam, sparkSession: SparkSession = null): DataFrame = {
    ...
    val node = StartDBParseDriver.parseSql(sql)
    ...
    try {
      node match {
        ...
        case _: SqlSelect =>
          StartSparkTableExtractVisitor.getTableList(sql).foreach { i =>
  ...
  def getSparkSession(isLocal: Boolean, redisConf: Option[RedisConf] = None): SparkSession = {
    val builder = SparkSession.builder().config(buildSparkConf(redisConf)).appName("Start-SPARK")
  ...
  def buildSparkConf(redisConf: Option[RedisConf]): SparkConf = {
    ...
    if (redisConf != null && redisConf.isDefined) {
