...
import org.urbcomp.start.db.flink.kafka.ThreadLocalCleaner;
import org.urbcomp.start.db.flink.processfunction.JoinProcess;
import org.urbcomp.start.db.flink.visitor.*;
...
import org.urbcomp.start.db.udf.AbstractUdf;
...
public class FlinkQueryExecutor {
    ...
    public DataStream<Row> execute(FlinkSqlParam param) {
        ...
        if (sqlNode == null) {
            sqlNode = StartDBParseDriver.parseSql(sql);
        }
        switch (sqlNode.getKind()) {
                    ...
                    try {
                        if (param.getTestNum() == 0) {
                            ...
                            long affectRow = MetadataAccessUtil.updateJobName(
                                MetadataUtil.parseJobIdFromTopic(param.getStreamResTopic()),
                                jobId
                            );
            ...
            case INSERT:
                ...
                if (param.isInsertStreamIntoDimension()) {
                    ...
                    sqlNodeCache.set(StartDBParseDriver.parseSql(selectSql));

                    DataStream<Row> resultStream = execute(flinkSqlParam);

                    // write result to kafka
                    org.urbcomp.start.db.metadata.entity.Table insertTable = getTable(
    ...
    public void loadTable(
        ...
        String tableName,
        String dbTableName,
        org.urbcomp.start.db.metadata.entity.Table table
    ) {
        // create kafkaSource
    ...
    public void loadTables(
        ...
        List<org.urbcomp.start.db.metadata.entity.Table> tableList
    ) {
    ...
    public void checkInsert(
        ...
        org.urbcomp.start.db.metadata.entity.Table insertTable
    ) {
    ...
    public static org.urbcomp.start.db.metadata.entity.Table getTable(String dbTable) {
    ...
    public static List<org.urbcomp.start.db.metadata.entity.Table> getTables(
        List<String> dbTableList
    ) {
        List<org.urbcomp.start.db.metadata.entity.Table> tableList = new ArrayList<>();
