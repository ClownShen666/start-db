...
public class JoinVisitor extends StartDBSqlBaseVisitor<Void> {
    ...
    // visit sql
    @Override
    public Void visitSubselectStmt(StartDBSqlParser.SubselectStmtContext ctx) {
        // query field
    ...
    private void parseField(List<StartDBSqlParser.SelectListItemContext> selectList) {
        for (StartDBSqlParser.SelectListItemContext selectListItem : selectList) {
    ...
    private void parseJoin(List<StartDBSqlParser.FromJoinClauseContext> joinClauseContexts) {
        for (StartDBSqlParser.FromJoinClauseContext joinClauseContext : joinClauseContexts) {
            Join join = new Join();
            ...
            if (joinClauseContext.fromJoinTypeClause() != null) {
                ...
                if (joinClauseContext.boolExpr().boolExpr().isEmpty()) {
                    ...
                    for (StartDBSqlParser.BoolExprContext boolExprContext : joinClauseContext
                        .boolExpr()
                        .boolExpr()) {
    ...
    private void parseOn(StartDBSqlParser.BoolExprContext boolExprContext) {
        if (boolExprContext.boolExprAtom().boolExprBinary() != null) {
            StartDBSqlParser.BoolExprBinaryContext binaryContext = boolExprContext.boolExprAtom()
                .boolExprBinary();
            On on = new On();
    ...
    private void parseWhere(StartDBSqlParser.WhereClauseContext whereClauseContext) {
        where = " where";
        if (whereClauseContext.boolExpr().boolExpr().isEmpty()) {
            parseBoolExpr(whereClauseContext.boolExpr());
        } else {
            boolean first = true;
            for (StartDBSqlParser.BoolExprContext boolExprContext : whereClauseContext.boolExpr()
                .boolExpr()) {
                if (first) {
                    first = false;
    ...
    // currently only support binary boolExpr and assume no function
    private void parseBoolExpr(StartDBSqlParser.BoolExprContext boolExprContext) {
        if (boolExprContext.boolExprAtom().boolExprBinary() != null) {
            StartDBSqlParser.BoolExprBinaryContext binaryContext = boolExprContext.boolExprAtom()
                .boolExprBinary();
