...
import org.start.db.geomesa.storage.curve.XZStarSFC
...
class XZStarTIndexKeySpace(
  ...
  override def toIndexKey(
    ...
    SingleRowKeyValue(bytes, sharing, shard, XZStarTIndexKey(b, xz), tier, id, writable.values)
  }

  override def getIndexValues(filter: Filter, explain: Explainer): XZStarTIndexValues = {
    ...
    if (intervals.disjoint) {
      ...
      return XZStarTIndexValues(sfc, geometries, Seq.empty, intervals, Seq.empty, Seq.empty)
    }
    ...
    XZStarTIndexValues(sfc, geometries, xy, intervals, timesByBin.result(), unboundedBins.result())
  ...
  override def getRanges(
    ...
    def toZRanges(): Seq[IndexRange] =
      xzstart.ranges(xy.map { case (xmin, ymin, xmax, ymax) => (xmin, ymin, xmax, ymax) }, target)
    ...
    val unbounded = unboundedBins.iterator.map {
      case (lower, Short.MaxValue) => LowerBoundedRange(XZStarTIndexKey(lower, 0L))
      case (0, upper)              => UpperBoundedRange(XZStarTIndexKey(upper, Long.MaxValue))
      case (lower, upper) =>
        logger.error(s"Unexpected unbounded bin endpoints: $lower:$upper")
        UnboundedRange(XZStarTIndexKey(0, 0L))
    }
...
object XZStarTIndexKeySpace extends IndexKeySpaceFactory[XZStarTIndexValues, XZStarTIndexKey] {
  ...
  override def apply(
    ...
    new XZStarTIndexKeySpace(sft, shards, attributes.head, attributes.last)
  }
}
